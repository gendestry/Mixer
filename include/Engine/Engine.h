...
public:
    ...
    std::vector<uint16_t> patch(const std::string &fixtureName, uint8_t universe, uint16_t amount,
                                std::optional<uint16_t> start = std::nullopt,
                                std::optional<uint16_t> startFID = std::nullopt);

    std::vector<uint16_t> patch(DMX::Fixture &fixture, uint8_t universe, uint16_t amount,
                                std::optional<uint16_t> start = std::nullopt,
                                std::optional<uint16_t> startFID = std::nullopt);
    ...
    Effect::FXDimmerGrouped *addEffectDimmerGroup(const std::string &groupName,
                                                  Utils::Curve::Type curve = Utils::Curve::SINUSOID) {
    ...
    Effect::FXDimmerChase *addEffectDimmerChase(const std::string &groupName,
                                                Utils::Curve::Type curve = Utils::Curve::SINUSOID) {
    ...
    Effect::FX2Color *addEffect2Color(const std::string &groupName, Utils::Colors::RGB color1,
                                      Utils::Colors::RGB color2) {
    ...
    Effect::FXColorGradient *addEffectColorGradient(const std::string &groupName,
                                                    const std::vector<Utils::Colors::RGB> &colors,
    ...
    [[nodiscard]] std::vector<std::shared_ptr<DMX::Parameters::Parameter> > &getGroupParameter(
        const std::string &name, DMX::Parameters::Type paramType);

    [[nodiscard]] std::vector<std::shared_ptr<DMX::Parameters::Parameter> > &getGroupColorParameter(
