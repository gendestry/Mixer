...
namespace Effect {
    ...
    struct ColorFanEffect : public Effect {
        uint16_t m_peaks;

        ColorFanEffect(DMX::FixtureGroup &group, Utils::Colors::RGB color1, Utils::Colors::RGB color2,
                       uint16_t peaks = 1)
            : Effect(group, Type::COLOR),
              m_peaks(peaks) {
            setEffect([color1, color2, this](uint32_t tick) {
                auto &params = this->m_parameters;
                const auto grad = Utils::Colors::gradient(color1, color2, params.size(), true);
                for (int i = 0; i < params.size(); ++i) {
                    auto &color = grad[(i) % (params.size() / m_peaks)];
    ...
    struct ColorGradientEffect : public Effect {
        ColorGradientEffect(DMX::FixtureGroup &group, const std::vector<Utils::Colors::RGB> &colors,
    ...
    struct FXColorGradient : public Effect {
        FXColorGradient(DMX::FixtureGroup &group, const std::vector<Utils::Colors::RGB> &colors,
                        const std::vector<float> &percentages)
            : Effect(group, Type::COLOR) {
            setEffect([colors, percentages, this](uint32_t tick) {
                auto &params = this->m_parameters;
